      ...
      {fields.map((f) => (
        <tr key={f.label} className={`border-t border-border text-[11.5px] hover:bg-accent/15 ${f.strong ? "text-foreground font-medium" : "text-dim"}`}>
          ...
          {periods.map((p, i) => {
            ...
            return (
              <td key={p.periodKey} className={`py-2 pr-6 text-right ${hover.className}`} onMouseEnter={hover.onMouseEnter} onMouseLeave={hover.onMouseLeave}>
                {f.strong ? (
    ...
    <>
      <tr className="cursor-pointer border-t border-border hover:bg-accent/15" onClick={() => toggleExpanded(rowKey)}>
        ...
        {periods.map((p, i) => {
          const hover = colHoverProps(i, col, setCol);
          return (
            <td key={p.periodKey} className={`py-2.5 pr-6 text-right ${hover.className}`} onMouseEnter={hover.onMouseEnter} onMouseLeave={hover.onMouseLeave}>
    ...
    <>
      <tr className="cursor-pointer border-t border-dim/25 hover:bg-accent/15" onClick={setSectionOpen}>
        ...
        {periods.map((p, i) => {
          const hover = colHoverProps(i, col, setCol);
          return (
            <td key={p.periodKey} className={`py-3 pr-6 text-right font-semibold ${hover.className}`} onMouseEnter={hover.onMouseEnter} onMouseLeave={hover.onMouseLeave}>
        ...
        <table className="w-full border-separate border-spacing-0 text-[12.5px] tabular-nums [&_thead_th]:sticky [&_thead_th]:top-0 [&_thead_th]:border-b [&_thead_th]:border-border [&_thead_th]:bg-panel-2 [&_thead_th:not(:first-child)]:z-20 [&_tbody_td:first-child]:sticky [&_tbody_td:first-child]:left-0 [&_tbody_td:first-child]:z-10 [&_tbody_td:first-child]:bg-panel [&_tbody_tr:hover>td:first-child]:!bg-[color-mix(in_srgb,var(--panel)_85%,var(--accent)_15%)] [&_td]:whitespace-nowrap [&_th]:whitespace-nowrap [&_tbody_td:first-child]:border-r [&_tbody_td:first-child]:border-border [&_tbody_td:first-child]:pr-6 [&_thead_th:first-child]:border-r [&_thead_th:first-child]:border-border [&_thead_th:first-child]:pr-6">
          <thead>
            <tr className="text-left text-[11px] text-dim">
              ...
              {periods.map((p, i) => (
                <th
                  key={p.periodKey}
                  className={`py-3 pr-6 text-right font-medium ${hoveredCol === i ? "bg-accent/10 text-foreground" : ""}`}
          ...
          <tbody>
            <tr className="border-b border-dim/25">
              <td className="py-3 pl-2 font-bold">Net Worth</td>
              {periods.map((p, i) => (
                <td
                  key={p.periodKey}
                  className={`py-3 pr-6 text-right font-bold ${hoveredCol === i ? "bg-accent/10" : ""}`}
                  onMouseEnter={() => setHoveredCol(i)}
                  onMouseLeave={() => setHoveredCol(null)}
                >
