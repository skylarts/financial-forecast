...
At year-end the engine computes the **exact** federal bill from real IRS bracket tables (brackets and the standard deduction are inflation-indexed forward from 2026): ordinary income = **gross salary (where a gross amount is entered)** + gross tax-deferred withdrawals and RMDs + Roth conversions + gross pension + the taxable portion of Social Security (per the IRS partial-inclusion rule), less the standard deduction; long-term capital gains are stacked on top of ordinary income in the LTCG brackets; early-withdrawal penalties are added on top. The optional flat add-on rate is applied to the same combined base to approximate state/local tax. **Each December the estimated withholding is settled against this exact bill** -- a refund (or extra charge) posted to Extra Savings -- so the household's actual cash tax for a year always equals the exact bracket bill, and the balances/net-worth trajectory reflect it. Because tax during the monthly loop depends on rates that depend on the year's income, the whole simulation is re-run a few times until each year's marginal-rate estimate converges on its actual result.

**Reading the tax numbers without fooling yourself.** Two traps, both easy to fall into:
1. *Every figure in the projection is NOMINAL, and so are the brackets.* The engine indexes the brackets and the standard deduction forward at the plan's own inflation rate, exactly as the IRS does. So $300,000 of taxable income in 2053 is **not** a $300,000 income in today's terms and does **not** sit in the bracket that $300,000 sits in today -- deflate it first (divide by the period's \`inflationDeflator\`/\`flowInflationDeflator\`) and read the bracket off the real figure. A household whose nominal income looks like it is deep in the 24% bracket decades out is very often in the 12-22% bracket in real terms.
2. *\`ordinaryTaxableIncome\` includes salary.* In any year with earned income that field is dominated by it, so it is not a measure of retirement income and a working year must not be compared against retired years. The federal tax column is the opposite -- it **excludes** tax on salary, because salary is entered take-home. So in a working year you will see a large taxable income next to a tiny tax bill; both are correct and they are measuring different things.

These two together decide whether Roth conversions help: the question is always whether the **real** marginal rate now is below the **real** marginal rate later, never whether the nominal numbers look big.
...
export function buildLlmExport(scenario: Scenario): string {
  ...
  lines.push("- **Stress test tab** — the plan re-run under one bad assumption at a time: lower returns, a bear market (one year or several), replays of 1929/1966/1973/2000/2008, higher inflation, a longer life, a spouse's early death, long-term care, a forced early exit, spending overruns, healthcare cost growth, higher taxes, a Social Security cut, a frozen pension; not saved with the plan.");
