...
describe("retirementDateOf", () => {
  it("is the birthday at the retirement age", () => {
    expect(retirementDateOf(alex)).toBe("2042-05-15");
  });

  it("prefers an exact date when one is set", () => {
    expect(retirementDateOf({ ...alex, retirementDate: "2041-06-30" })).toBe("2041-06-30");
  });

  it("is null for someone modelled as never retiring", () => {
    expect(retirementDateOf({ ...alex, skipRetirement: true })).toBeNull();
  });
});
...
describe("resolveAnchoredDates", () => {
  ...
  it("moves every linked date when a retirement age changes -- the whole point", () => {
    const base = scenario({
      ...
      events: [{ startDate: "x", endDate: "y", startAnchor: at("p1"), endAnchor: at("p1", 180) }],
    });
  ...
  it("follows an exact retirement date when the person has one", () => {
    const next = resolveAnchoredDates(
      scenario({
        household: { people: [{ ...alex, retirementDate: "2040-01-01" }, jordan] },
  ...
  it("leaves a date linked to someone who never retires alone", () => {
    const next = resolveAnchoredDates(
      scenario({
        household: { people: [{ ...alex, skipRetirement: true }, jordan] },
        incomeSources: [{ startDate: "2030-01-01", endDate: null, startAnchor: at("p1") }],
      })
    );
    expect(next.incomeSources[0].startDate).toBe("2030-01-01");
  });
});
...
describe("countAnchorsToPerson", () => {
  it("counts both ends of every linked item, and only for that person", () => {
    const s = {
      ...
      events: [{ startDate: "x", startAnchor: at("p1") }],
    };
    expect(countAnchorsToPerson(s, "p1")).toBe(3);
