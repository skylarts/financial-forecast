...
**Retirement is a property of a person, not an event.** Each person has a retirement age (and, if they need one, an exact date); that is what stops their salary and paycheck contributions, switches their healthcare stage, places the retirement KPIs and the chart's milestone, and anchors any linked date. There used to be a separate Retire *event* carrying the real date, with the age merely seeding it — so a person added without one worked to the end of the plan in silence. Loading an older plan folds each retire event onto its person and drops it (schema 5 → 6), keeping an exact date only when the age could not reproduce it. `skipRetirement` models someone working straight through, and their retirement spending — the extra travel and hobbies — starts the day they retire and moves with it.
